# 🎋 Bamboocut – Wind

**Rolle:** Aggro-Burst-DPS · **Schwierigkeit:** ⭐⭐⭐⭐ · **Non-Stop Aggression**

Der Wind-Pfad von Bamboocut ist pure Aggression. Extrem schnelle Angriffe, hohe kritische Treffer und ein Burst-Modus, der Gegner in Sekunden zerlegt. Nichts für schwache Nerven.

---

## Inferno-Zwillingsklingen (Infernal Twinblades) ⭐ Burst-DPS

| Eigenschaft | Details |
|------------|---------|
| **Kampfart** | Nahkampf |
| **Rolle** | DPS |
| **Reichweite** | Kurz |

### Stärken & Schwächen

| Stärken | Schwächen |
|---------|-----------|
| ✅ Extrem hoher Burst-Schaden | ❌ Sehr geringe Überlebensfähigkeit |
| ✅ Flamelash-Verwandlung | ❌ Erfordert aggressives Spiel |
| ✅ Lebensraub im Burst-Modus | ❌ Kurze Reichweite |
| ✅ Hohe kritische Trefferrate | |

### Schlüssel-Skills

- **Addled Mind (Verwirrter Geist)** – Wirbelnder Dash-Angriff (3 Aufladungen)
- **Calamity's Greed (Gier des Unheils)** – Klinge hochwerfen + kreiseln lassen + "Enlightened"-Zustand
- **Flamelash (Flammenpeitsche)** – **Burst-Modus**: verstärkte Angriffe + Lebensraub

### Ressource
- **Blazing Wrath (Kampfwut)** – Füllt sich bei aggressiven Aktionen → aktiviert Flamelash

---

## Sterblicher Seilpfeil (Mortal Rope Dart)

| Eigenschaft | Details |
|------------|---------|
| **Kampfart** | Hybrid |
| **Rolle** | DPS |
| **Reichweite** | Mittel |

### Stärken & Schwächen

| Stärken | Schwächen |
|---------|-----------|
| ✅ Flexible Distanzüberbrückung | ❌ Komplexe Seil-Mechanik |
| ✅ Gute Verfolgung | ❌ Weniger Burst als Zwillingsklingen |
| ✅ Hybrid aus Nah- und Fernkampf | |

### Mechaniken
- Seilangriffe für flexible Distanzkontrolle
- Heranziehen an Gegner oder Flucht

---

## Spielstil

**Inferno-Zwillingsklingen** sind eine der stärksten DPS-Waffen im Spiel, aber erfordern perfektes Timing. Der Kern ist der **Flamelash-Burst**.

### Empfohlene Rotation
1. Addled Mind (3x aufladen, rein-dash)
2. Calamity's Greed (Enlightened-Zustand)
3. ⚡ **Flamelash aktivieren** – jetzt maximalen Schaden fahren

---

## ⚙️ Build-Empfehlung: Bamboocut – Wind

### Empfohlene Innere Kampfkünste (Mystic Arts)

| Mystic Art | Wirkung | Priorität |
|-----------|---------|:---------:|
| **Guardian Palm** ⭐ | Schadensschild – überlebenskritisch für aggressiven Spielstil | ⭐⭐⭐ |
| **Meridian Touch** | Unterbrechungen + Projektil-Konter | ⭐⭐⭐ |
| **Cloud Steps** | Extra Ausweichen für Risiko-Management | ⭐⭐ |
| **Blazing Wrath Boost** | Passt den Stil an, um Kampfwut schneller zu füllen | ⭐⭐ |

### Boss-Matchups

| Boss | Empfehlung | Begründung |
|------|-----------|-----------|
| **Heartseeker** | ✅ Stark | Phase 1 schnell bursten – Phase 2 vorsichtig! |
| **Qianye** | ⚠️ Mittel | Ihre schnellen Angriffe können dich in Flamelash unterbrechen |
| **Ye Wanshan** | ✅ Stark | Kurzer Burst-Fight – ideal für Twinblades |
| **Weltbosse** | ✅✅ Exzellent | Die meisten Weltbosse fallen dem Flamelash-Burst zum Opfer |

### Stat-Priorität
`Angriffskraft > Kritischer Schaden > Kritische Trefferrate > Angriffsgeschwindigkeit`


**Inferno-Zwillingsklingen** sind eine der stärksten DPS-Waffen im Spiel, aber erfordern perfektes Timing. Der Kern ist der **Flamelash-Burst** – sobald Blazing Wrath voll ist, verwandelst du dich und rottest alles nieder.

### Empfohlene Rotation
1. Addled Mind (3x aufladen, rein-dash)
2. Calamity's Greed (Enlightened-Zustand)
3. ⚡ **Flamelash aktivieren** – jetzt maximalen Schaden fahren
4. Wiederholen
