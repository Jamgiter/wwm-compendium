# 🪨 Stonesplit – Might

**Rolle:** Tank · **Schwierigkeit:** ⭐⭐⭐ · **Der unzerstörbare Fels**

Der Tank-Pfad von Stonesplit. Bietet **Schilde, Schadensreduktion, Taunt** und die Fähigkeit, Gegner zu gruppieren. Unverzichtbar für schwere Gruppen-Inhalte, aber etwas geringerer Schaden als reine DPS-Pfade.

---

## Sturmbrecher-Speer (Stormbreaker Spear) ⭐ Main Tank

| Eigenschaft | Details |
|------------|---------|
| **Kampfart** | Nahkampf |
| **Rolle** | Tank |
| **Reichweite** | Lang |

### Stärken & Schwächen

| Stärken | Schwächen |
|---------|-----------|
| ✅ Exzellente Überlebensfähigkeit | ❌ Geringe Mobilität |
| ✅ Taunt (Bedrohung) | ❌ Kaum Support |
| ✅ Gegner gruppieren | ❌ Mittlerer Schwierigkeitsgrad |
| ✅ Schadensreduktion | |

### Schlüssel-Skills

- **Storm Roar (Sturmgebrüll)** – Schallwellen + **Taunt** + Schadensreduktion
- **Thunder Shock (Donnerschock)** – Wirbeltritt + Aufschlagger + "Vulnerability"-Debuff
- **Fury Spear (Zornspeer)** – Aufgeladener Stoßangriff mit "Drumbeat"-Buff

### Ressource
- **Fighting Spirit (Kampfgeist)** – Wird durch Blocken/Taunt aufgeladen

---

## Donnerklage-Klinge (Thundercry Blade) ⭐ Off-Tank

| Eigenschaft | Details |
|------------|---------|
| **Kampfart** | Nahkampf |
| **Rolle** | Tank / DPS |
| **Reichweite** | Mittel |

### Stärken & Schwächen

| Stärken | Schwächen |
|---------|-----------|
| ✅ Schilde | ❌ Weniger Bedrohung als Speer |
| ✅ Konter-Mechaniken | ❌ Ressourcen-Intensiv |
| ✅ Hohe Verteidigung | |

### Schlüssel-Skills

- **Sunrush Gale (Sonnensturm-Böe)** – Mobilitäts-Angriff
- **Predator's Shield (Räuberschild)** – Schild + Schadensreduktion
- **Avalanche (Lawine)** – Aufgeladener AoE-Angriff

### Ressource
- **Battle Will (Kampfwille)** – Ressource für verstärkte Angriffe und Schilde

---

## Spielstil

Als **Tank** bist du die Frontlinie. Der Sturmbrecher-Speer ist deine Hauptwaffe für Bedrohung und Gruppierung. Wechsle zur Donnerklage-Klinge, wenn du mehr Konter brauchst oder der Speer gerade abklingt.

### Empfohlene Rotation
1. Storm Roar (Aggro holen + Schadensreduktion)
2. Gegner gruppieren
3. Fury Spear (aufgeladener Schaden)
4. Thunder Shock (Vulnerability-Debuff für Gruppe)
5. Bei Bedarf: Thundercry Blade-Wechsel für Konter

---

## ⚙️ Build-Empfehlung: Stonesplit – Might

### Empfohlene Innere Kampfkünste (Mystic Arts)

| Mystic Art | Wirkung | Priorität |
|-----------|---------|:---------:|
| **Guardian Palm** ⭐ | Extra-Schadensschild + Schadensreduktion | ⭐⭐⭐ |
| **Meridian Touch** | Boss unterbrechen, um Gruppe zu schützen | ⭐⭐⭐ |
| **Cloud Steps** | Mobilität für Positionierung | ⭐⭐ |
| **Leaping Toad** | Zusätzliche AoE-Bedrohung | ⭐ |

### Boss-Matchups

| Boss | Empfehlung | Begründung |
|------|-----------|-----------|
| **Heartseeker** | ✅✅ Stark | Taunt unterbricht Blitzsturm-Vorbereitung? |
| **Twin Lions** | ✅✅✅ **Idealer Tank** | Zwei Gegner gleichzeitig taunten |
| **Jeder Gruppen-Boss** | ✅ Pflicht | Unverzichtbar in Hard-Content-Gruppen |

### Stat-Priorität
`Verteidigung > Ausdauer > Bedrohung > Schadensreduktion`

1. Storm Roar (Aggro holen + Schadensreduktion)
2. Gegner gruppieren
3. Fury Spear (aufgeladener Schaden)
4. Thunder Shock (Vulnerability-Debuff für Gruppe)
5. Bei Bedarf: Thundercry Blade-Wechsel für Konter
